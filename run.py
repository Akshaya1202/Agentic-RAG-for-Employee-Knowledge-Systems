...
def main():
    ...
    for i, sample in enumerate(data, start=1):
        ...
        try:
            ...
            print(f"  Route: {rag_response.get('route')}")
            if rag_response.get("sql"):
                print(f"  SQL: {rag_response.get('sql')}")
                print(f"  Rows: {rag_response.get('rows')}")
            if rag_response.get("retrieval"):
                print(f"  Retrieval: {rag_response.get('retrieval')}")
